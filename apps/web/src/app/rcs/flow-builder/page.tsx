...
export default function RcsFlowBuilderPage() {
  ...
  const handleCreateFlow = async (e: React.FormEvent) => {
    ...
    if (!name || !triggerKeyword || !stepText) {
      toast.error('Please enter flow name, trigger keyword, and step message');
      return;
    }

    setSaving(true);
    try {
      const res = await api.post('/rcs/flows', {
        name,
        triggerKeyword,
        flowData: {
          steps: [
      ...
      if (res.data?.status) {
        toast.success('RCS Interactive Bot Flow created!');
        setName('');
        setTriggerKeyword('');
        setStepText('');
        setShowAdd(false);
  ...
  const handleDeleteFlow = async (id: number) => {
    try {
      await api.delete(`/rcs/flows/${id}`);
      toast.success('RCS flow deleted');
      fetchFlows();
    } catch (err) {
  ...
  if (loading) return <div className="flex items-center justify-center h-full">Loading RCS flows...</div>;

  return (
    <div className="max-w-4xl mx-auto space-y-8 py-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Google RCS Flow Builder</h2>
          <p className="text-gray-600 mt-1">Design multi-step interactive bot flows and quick reply chips for Google RCS</p>
        </div>
        ...
        >
          <Plus size={18} />
          <span>New Interactive RCS Flow</span>
        </button>
      </div>

      {showAdd && (
        <div className="bg-white p-6 rounded-2xl border shadow-md space-y-4">
          <h3 className="text-lg font-bold text-gray-900 border-b pb-2">Create RCS Interactive Bot Flow</h3>
          <form onSubmit={handleCreateFlow} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Product Catalog Bot"
                  value={name}
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. catalog, menu, start"
            ...
            >
              {saving ? 'Creating Flow...' : 'Create RCS Flow'}
            </button>
          </form>
        </div>
      )}
      ...
      <div className="space-y-4">
        {flows.length === 0 ? (
          <div className="bg-white p-12 text-center rounded-2xl border text-gray-500 shadow-sm">
            ...
            <h4 className="font-bold text-gray-800">No Interactive RCS Flows</h4>
            <p className="text-xs text-gray-400 mt-1">Click "New Interactive RCS Flow" to create one.</p>
          </div>
        ) : (
          flows.map((f) => (
