...
export default function RcsCampaignsPage() {
  ...
  const [templateName, setTemplateName] = useState('RCS Rich Card');
  ...
  const fetchCampaigns = async () => {
    try {
      const res = await api.get('/rcs/campaigns');
      if (res.data?.status && Array.isArray(res.data.result)) setCampaigns(res.data.result);
    } catch (err) {
      toast.error('Failed to load RCS campaigns');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateCampaign = async (e: React.FormEvent) => {
    ...
    if (!name || numbers.length === 0 || !scheduledTime) {
      toast.error('Please fill in campaign name, numbers, and schedule time');
      return;
    }

    setSaving(true);
    try {
      const res = await api.post('/rcs/campaigns', {
        name,
        templateName,
        numbers,
      ...
      if (res.data?.status) {
        toast.success('RCS Campaign scheduled!');
        setName('');
        setNumbersText('');
        setScheduledTime('');
        setShowAdd(false);
  ...
  const handleDeleteCampaign = async (id: number) => {
    try {
      await api.delete(`/rcs/campaigns/${id}`);
      toast.success('RCS campaign deleted');
      fetchCampaigns();
    } catch (err) {
  ...
  if (loading) return <div className="flex items-center justify-center h-full">Loading RCS campaigns...</div>;

  return (
    <div className="max-w-4xl mx-auto space-y-8 py-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">RCS Scheduled Campaigns</h2>
          <p className="text-gray-600 mt-1">Schedule Google RCS Rich Broadcasts to target phone lists</p>
        </div>
      ...
      {showAdd && (
        <div className="bg-white p-6 rounded-2xl border shadow-md space-y-4">
          <h3 className="text-lg font-bold text-gray-900 border-b pb-2">Schedule RCS Broadcast Campaign</h3>
          <form onSubmit={handleCreateCampaign} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Festive RCS Offer"
                  value={name}
            ...
            >
              {saving ? 'Scheduling...' : 'Schedule RCS Campaign'}
            </button>
          </form>
        </div>
      )}
      ...
      <div className="space-y-4">
        {campaigns.length === 0 ? (
          <div className="bg-white p-12 text-center rounded-2xl border text-gray-500 shadow-sm">
            ...
            <h4 className="font-bold text-gray-800">No Scheduled RCS Campaigns</h4>
            <p className="text-xs text-gray-400 mt-1">Click "New RCS Campaign" to create one.</p>
          </div>
        ) : (
          campaigns.map((c) => (
